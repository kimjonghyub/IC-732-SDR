...
op.set_defaults(
    buffers                 = 6,       # no. buffers in sample chunk (RPi-40)
    control_si570           = False,    # normally, talk to RTL or Hamlib for freq info
    cpu_load_interval       = 10,      # cycle time for CPU monitor thread
    ...
    hamlib_interval         = 0.05,      # Wait between hamlib freq. checks (secs)    
    hamlib_rigtype          = 3021,      # FT857==122. IC-706==309. IC-737==3021.
    ...
    rtl_frequency           = 69011500,   # RTL center freq. Hz  #IC-732 IF Freq
    rtl_gain                = 45,        # auto
    ...
    skip                    = 100,        # if not =0, skip some input data
    source_rtl              = False,    # Use sound card, not RTL-SDR input
    sp_min                  =-140,      # dB relative to clipping, at bottom of grid
    sp_max                  = -10,       # dB relative to clipping, at top of grid
    v_min                   =-90,      # palette starts at this level
    v_max                   =-10,       # palette ends at this level
    waterfall               = True,    # Using waterfall? T/F
    waterfall_accumulation  = 2,        # No. of spectra per waterfall line
...
opt.ident = "IC732 Panadapter de DS3DND"
