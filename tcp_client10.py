...
def draw_level_meter(screen, center_magnitude, meter_rect):
    ...
    # Normalize the center_magnitude value to the meter width
    min_dB = 0  # Minimum decibel level
    max_dB = 60  # Maximum decibel level
    normalized_value = (smoothed_center_magnitude - min_dB) / (max_dB - min_dB)
    normalized_value = np.clip(normalized_value, 0, 1)  # Ensure value is between 0 and 1

    # Draw the meter background
    pygame.draw.rect(screen, GRAY, meter_rect)

    # Calculate the filled width of the meter
    ...
    # Display the center magnitude value as text
...
def smooth_fft_graph(freqs, magnitudes, num_points=500):
    # Create smoother frequency and magnitude arrays
    x_smooth = np.linspace(freqs.min(), freqs.max(), num_points)
    spline = make_interp_spline(freqs, magnitudes, k=3)  # Cubic spline
...
def draw_fft_graph(screen, freqs, magnitudes):
    ...
    num_xticks = 4  # Number of ticks
    tick_spacing = graph_width // num_xticks  # Pixel spacing between ticks
    
    for i in range(num_xticks + 1):
        x_pos = i * tick_spacing
        freq = int((i - num_xticks // 2) * (max_freq / num_xticks) / 1e3)  # Frequency in kHz
    ...
    for i in range(num_yticks):
        ...
        pygame.draw.line(
            ...
            (graph_x+graph_width, int(y_tick)),
            1
        )
    ...
    # Calculate x coordinate for the center frequency
    ...
    # Get the magnitude at the center frequency
