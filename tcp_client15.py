...
def init_audio():
    ...
    device_index = 1  # Set the output device index to 1
...
def am_modulate(iq_signal, carrier_freq, sample_rate):
    """
    Modulate the IQ signal to an AM audio signal.
    :param iq_signal: IQ complex signal
    :param carrier_freq: Frequency of the carrier (Hz)
    :param sample_rate: Sampling rate of the IQ signal (Hz)
    :return: AM-modulated audio signal
    """
    t = np.arange(len(iq_signal)) / sample_rate
    carrier = np.cos(2 * np.pi * carrier_freq * t)
    amplitude_modulated = (1 + MODULATION_INDEX * np.abs(iq_signal)) * carrier
    return amplitude_modulated.astype(np.float32)
    
async def audio_output_task(iq_queue, stream):
    while True:
        try:
            iq_data = await iq_queue.get()
            complex_signal = process_iq_data(iq_data)
            complex_signal = remove_dc_component(complex_signal)
            audio_signal = am_modulate(complex_signal, carrier_freq=1200, sample_rate=SAMPLE_RATE)
            resampled_audio_signal = scipy.signal.resample_poly(audio_signal, up=AUDIO_SAMPLE_RATE, down=SAMPLE_RATE)
            
            stream.write(resampled_audio_signal.tobytes())
...
async def main():
    ...
    running = True
    #audio, stream = init_audio()  # Initialize audio system
            ...
            if event.type == pygame.QUIT:
                ...
                if button_rect.collidepoint(event.pos):
                    if is_connected:
                        ...
                        await toggle_connection()
                        #if is_connected:
                        #tasks.append(asyncio.create_task(audio_output_task(data_queue, stream)))    
    ...
    #close_audio(audio, stream)  # Close the audio system
    pygame.quit()
