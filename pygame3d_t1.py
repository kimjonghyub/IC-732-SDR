...
gluPerspective(70, (display[0] / display[1]), 0.01, 3.5)
glTranslatef(-2.75, -1.05, -2)
glRotatef(-60, 1, 0, 0)
glRotatef(0, 0, 0, 1)
...
def draw_surface(X, Y, Z, x_left, x_right, z_wall_fixed=5):
    draw_grid_walls(x_left, x_right, z_fixed=z_wall_fixed, grid_spacing=0.2, height=3)
...
try:
    ...
    while running:
        ...
        # Update data
        X, Y, Z = update_data(X, Y, Z, normalized_fft, step=0.08)
        ...
        for i, freq in enumerate(np.linspace(min_freq, max_freq, 6) / 1000):  
            draw_text_opengl((i * 1.032, 0.05, -0.57), f"{freq:.1f} kHz", color=(1, 1, 1))
