...
rig_command_queue = asyncio.Queue(maxsize=5)
rig_response_queue = asyncio.Queue(maxsize=5)
...
freq_dot_font = pygame.font.SysFont(None, 50)
freq_mhz_font = pygame.font.SysFont(None, 30)
freq_font = pygame.font.Font("Digital Display.ttf", 40)
...
async def rig_reader_task():
    global rig_reader, rig_writer, rig_connected, rig_frequency
    try:
        while rig_connected:
            response_event = None 
            try:
                ...
                print(f"Received frequency change: {rig_frequency} Hz")
                ...
                print(f"Incomplete read error: {e}. Connection may be closed.")
                rig_connected = False
                break  
            except Exception as e:
                print(f"Error in RIG reader task: {e}")
                if response_event:
                    response_event.set()
...
async def get_rig_frequency():
    try:
        print("Sending command to get rig frequency...")
        response = await send_rig_command(CMD_RIG_GET_FREQ, 0)
        if response:
            rig_frequency = struct.unpack(">I", response)[0]
            print(f"Rig frequency received: {rig_frequency} Hz")
        else:
            print("Failed to get frequency: No response from server.")
    except Exception as e:
        print(f"Error in get_rig_frequency: {e}")
            ...
            if event.type == pygame.QUIT:
                ...
                if button_rect.collidepoint(event.pos):
                    if is_connected:
                        ...
                        await toggle_connection()
                        #if is_connected:
                            #task = asyncio.create_task(get_rig_frequency())
                            #tasks.append(task)
