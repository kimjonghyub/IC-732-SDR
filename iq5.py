...
def draw_level_meter(bgcolor, color,audio_data, x, y, width, height):
    try:
        ...
        meterbar = pg.image.load('meter4.png').convert_alpha()
        meter = pg.image.load('meter6.png').convert_alpha()
        surf_main.blit(meterbar,(20,160))
        surf_main.blit(meter, (20,160),(0,0,bar_width,50))
    except Exception as e:
        print(f"Error in draw_level_meter_safe: {e}")
...
h_2d = 2*SCREEN_SIZE[1] / 9.6 if opt.waterfall \
...
lgfont = pg.font.Font("Patopian 1986.ttf", 80) # normal 16
lgfont_ht = lgfont.get_linesize()       # text height
mefont = pg.font.Font("BITSUMIS.TTF", 20)
mefont_ht = mefont.get_linesize()

mzfont = pg.font.Font("BITSUMIS.TTF", 24)
...
while True:
    ...
    if showfreq:
        ...
        if freq_step == 10000000:
            msg = "=="  
            surf_main.blit(mefont.render(msg, 1, BLACK, SCREEN), (int(w_middle)-136,int(y_2d)-170)) #10Mhz
        elif freq_step == 1000000:
            msg = "=="  
            surf_main.blit(mefont.render(msg, 1, BLACK, SCREEN), (int(w_middle)-96,int(y_2d)-170)) #1mHz
        elif freq_step == 100000:        
            msg = "=="  
            surf_main.blit(mefont.render(msg, 1, BLACK, SCREEN), (int(w_middle)-24,int(y_2d)-170))  #100kHz
        elif freq_step == 10000:
            msg = "=="  
            surf_main.blit(mefont.render(msg, 1, BLACK, SCREEN), (int(w_middle)+24,int(y_2d)-170))  #10kHz
        elif freq_step == 1000:
            msg = "=="  
            surf_main.blit(mefont.render(msg, 1, BLACK, SCREEN), (int(w_middle)+72,int(y_2d)-170))  #1kHz
        elif freq_step == 100:
            msg = "=="  
            surf_main.blit(mefont.render(msg, 1, BLACK, SCREEN), (int(w_middle)+140,int(y_2d)-170))  #0.1kHz
        elif freq_step == 10:
            msg = "=="  
            surf_main.blit(mefont.render(msg, 1, BLACK, SCREEN), (int(w_middle)+188,int(y_2d)-170))  #0.01kHz
        
        if opt.source=="rtl":
            mic_data = get_mic_data()
            fft_level(BLACK,SCREEN, mic_data, 20, 80, 160, 56)
    ...
    if opt.hamlib:
        ...
        button("STEP",(w_spectra-436),170,96,40,SCREEN,SCREEN,GRAY,WHITE,update_ts)
        button("BAND",(w_spectra-356),170,96,40,SCREEN,SCREEN,GRAY,WHITE,change_band)
        button("MODE",(w_spectra-276),170,96,40,SCREEN,SCREEN,GRAY,WHITE,change_mode)
        button("VFO A/B",(w_spectra-180),170,96,40,SCREEN,SCREEN,GRAY,WHITE,toggle_vfo)
        button("QUIT",(w_spectra-76),170,80,40,SCREEN,SCREEN,GRAY,WHITE,quit_all)
    else: button("QUIT",(w_spectra-76),170,80,40,SCREEN,SCREEN,GRAY,WHITE,quit_all)
